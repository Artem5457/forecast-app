...
import {getFiveDaysForecastSuccess, getForecast, getForecastSuccess} from "../actions/forecast-app.action";
  ...
  getTodayForecast$: Observable<Action> = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap((coords: LngLat) =>
        this.getForecastService.getForecastForCurrentMoment(coords).pipe(
